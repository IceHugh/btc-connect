...
export function useBalance() {
  ...
  watch(() => ctx.isConnected.value, async (isConnected) => {
        ...
        if (adapter) {
          ...
          if (typeof (adapter as any).getBalance === 'function') {
            try {
              ...
              const validBalance = balance && typeof balance === 'object' ? balance : null;
              ...
              if (validBalance && (adapter as any).state?.currentAccount) {
                ...
                if ((adapter as any).state.accounts && (adapter as any).state.accounts.length > 0) {
  ...
  const balance = computed(() => {
    ...
    const finalBalance = localBalance.value ||
  ...
  return {
    ...
    confirmedBalance: computed(() => {
      ...
      return currentBalance && typeof currentBalance === 'object' ? currentBalance.confirmed || 0 : 0;
    }),
    unconfirmedBalance: computed(() => {
      const currentBalance = balance.value;
      return currentBalance && typeof currentBalance === 'object' ? currentBalance.unconfirmed || 0 : 0;
    }),
    totalBalance: computed(() => {
      const currentBalance = balance.value;
      return currentBalance && typeof currentBalance === 'object' ? currentBalance.total || 0 : 0;
    }),
  };
}