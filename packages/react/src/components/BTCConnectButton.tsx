...
import { formatAddress, formatDecimal, formatBTCBalance } from '../utils';
...
export const BTCConnectButton: React.FC<BTCConnectButtonProps> = ({
  ...
  const { accounts, currentAccount, balance, isConnected, disconnect, theme } = useWallet();
  ...
  // 创建复制成功图标
  const CopiedIcon = () => (
    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="btc-copied-icon">
  ...
  const getButtonStyles = () => {
    const baseStyles: React.CSSProperties = {
      ...(size === 'sm' && { minHeight: '32px', padding: '4px 12px', fontSize: '12px' }),
      ...(size === 'lg' && { minHeight: '48px', padding: '12px 24px', fontSize: '16px' }),
      ...style,
    };
      ...
      >
        <div className="btc-balance-section">
          <p className="btc-balance-amount">
            {formatBTCBalance(balance?.total || 0)} <span className="btc-balance-unit">BTC</span>
          </p>
        </div>

        <div
          className="btc-address-section"
        ...
        >
          {copied ? (
            ...
            <p className="btc-address-text">{formatAddress(currentAccount.address, 4)}</p>
          )}
        </div>
...
export default BTCConnectButton;